...
- output
  - publicPath: 告知index.html内部引用资源的路径，路径规则：域名 + publicPath + filename
- devServer
  - publicPath: 指定本地服务所在的目录
  - contentBase: 打包之后的资源，如果依赖了其它资源，就告知去哪找
...
module.exports = {
  ...
  output: {
    ...
    path: path.resolve(__dirname, "dist"), // 路径，只能是绝对路径，打包之后输出到哪个目录
    // assetModuleFilename: "img/[name].[hash:4][ext]", // asset模块使用，这是全局配置，可能字体也使用 asset 模块
    // publicPath: '', // 默认值就是空字符串，不写浏览器会自动补全斜杠
    publicPath: '/', // 默认值就是空字符串
  },
  target: "web", // 开发阶段屏蔽掉，如果不写这个，热更新不生效，mode模式和.browserslistrc导致的
  devServer: {
    hot: true, // 这就相当于实现了热更新
    publicPath: '/lg' // 现在所有打包的东西都放在了 lg 下面了（一个虚拟目录），output的publicPath也需要改成lg，然后访问 域名/lg
